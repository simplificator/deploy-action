...
export async function run(): Promise<void> {
  try {
    ...
    execSync(`docker context create target --docker "host=ssh://${sshUserAtHost}:${sshPort}"`, { stdio: [] })
    ...
    execSync(`docker stack deploy --compose-file ${composeFile} --prune --with-registry-auth ${stackName}`, { stdio: [] })

    core.info("Waiting for deployment to complete ...")
    execSync(`docker run --rm -i -v $(pwd)/${composeFile}:/docker-compose.yml -v /var/run/docker.sock:/var/run/docker.sock sudobmitch/docker-stack-wait:v0.2.5 -l "--since 2m" -t 120 ${stackName}`, { stdio: [] })
